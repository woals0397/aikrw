...
*Setup instructions for a Gitian build of Aikrw using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aikrw
...
Clone the git repositories for aikrw and Gitian and then checkout the aikrw version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aikrw-project/aikrw.git
cd aikrw
...
Follow the instructions in [doc/release-process.md](release-process.md) in the aikrw repository
...
To build Aikrw (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the aikrw repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aikrw/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/aikrw-project/aikrw
...
For example:
```bash
URL=https://github.com/user/aikrw.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aikrw=${COMMIT} --url aikrw=${URL} ../aikrw/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aikrw=${COMMIT} --url aikrw=${URL} ../aikrw/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aikrw=${COMMIT} --url aikrw=${URL} ../aikrw/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aikrw-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aikrw-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/aikrw-build.assert
```
...
[aikrw/gitian.sigs](https://github.com/aikrw-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
